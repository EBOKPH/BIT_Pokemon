...
export const HABITAT_PROFILES = {
  ...
  ghost: {
    ...
    base_rate:     25,
    density:       'high',
    dead_zone:     0.30, // ghost zone goes completely silent frequently
    cooldown_min:  3,
    cooldown_max:  7,
    rare_chance:   0.30, // highest rare rate — ghost zone is volatile
...
export const RARITY = {
  COMMON:    { label: 'COMMON',    weight: 0.60, color: 'var(--green)',  xpMult: 1.0 },
  UNCOMMON:  { label: 'UNCOMMON',  weight: 0.25, color: '#80e8b0',       xpMult: 1.3 },
  RARE:      { label: 'RARE',      weight: 0.10, color: 'var(--blue)',   xpMult: 1.8 },
...
export function getTimeModifier() {
  ...
  if (h >= 0  && h < 6)  return { mult: 0.80, label: 'DEAD OF NIGHT', ghostBoost: 2.0 };
  // 06:00–09:59 — dawn: slightly boosted
  if (h >= 6  && h < 10) return { mult: 1.10, label: 'DAWN SCAN',     ghostBoost: 1.0 };
  // 10:00–16:59 — daylight: normal
  if (h >= 10 && h < 17) return { mult: 1.00, label: 'DAYLIGHT',      ghostBoost: 0.6 };
  // 17:00–20:59 — dusk: hot zone, best scan window
  if (h >= 17 && h < 21) return { mult: 1.30, label: 'DUSK WINDOW',   ghostBoost: 1.4 };
  // 21:00–23:59 — night: ghost/psychic surge, others slow
  return                         { mult: 0.90, label: 'NIGHT STATIC',  ghostBoost: 1.8 };
}
...
export function rollRarity(habitatRareChance) {
  ...
  else if (r < 0.40 + boost * 0.5)  return RARITY.UNCOMMON;
